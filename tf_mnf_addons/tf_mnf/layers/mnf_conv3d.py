...
class MNFConv3D(tf.keras.layers.Layer):
    """Bayesian 3D convolutional layer with weight posterior modeled by diagonal
    ...
    def build(self, input_shape):
        ...
        n_rows, n_cols, n_deep = self.kernel_size
        n_filters = self.n_filters
        self.input_dim = n_cols * stack_size * n_rows *n_deep
        W_shape = (n_rows, n_cols, n_deep, stack_size, n_filters)
        
        glorot =  tf.keras.initializers.GlorotNormal(seed=0)
    ...
    def call(self, x):
        ...
        mean = tf.nn.conv3d(input=x, filters=self.mean_W,padding=self.padding,strides=[1]+self.strides+[1]) + self.mean_b
        var = tf.nn.conv3d(input=tf.square(x), filters=var_w,padding=self.padding,strides=[1]+self.strides+[1]) + var_b

        mu_out = mean * z_samples[:, None, None, None,:]  # insert singleton dims
